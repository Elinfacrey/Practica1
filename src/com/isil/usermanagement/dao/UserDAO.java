...
public class UserDAO {
	...
	public void insertUser(Alumno user) throws SQLException {
		...
		try (Connection connection = Conexion.getConnection();
			...
			preparedStatement.setDouble(3, user.getAverage());
	...
	public Alumno selectUser(int id) {
		...
		try (Connection connection = Conexion.getConnection();
			...
			while (rs.next()) {
				...
				Alumno = new Alumno(id, name, lastname, average);
			}
		} catch (SQLException e) {
			printSQLException(e);
		}
	...
	public List<Alumno> selectAllUsers() {
		...
		try (Connection connection = Conexion.getConnection();
			...
			while (rs.next()) {
				...
				String lastname = rs.getString("lastname");
				double average = rs.getDouble("average");
				users.add(new Alumno(id, name, lastname, average));
			}
		} catch (SQLException e) {
			printSQLException(e);
		}
	...
	public boolean updateUser(Alumno user) throws SQLException {
		...
		try (Connection connection = Conexion.getConnection();
			...
			statement.setDouble(3, user.getAverage());
